...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            ...
            if normalization:
                self.params['gamma'+str(i+1)]=np.zeros(D)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization:
                ...
                fc, fc_cache = affine_forward(input_X, W, b)
                #bn, bn_cache = layernorm_forward(fc, gamma, beta, bn_param)
                relu, relu_cache = relu_forward(fc)
                input_X,dropout_cache=dropout_forward(relu,self.dropout_param)
                cache[i]= (fc_cache, relu_cache,dropout_cache)
            else:
